 ...
 *  **SAC** (S5a) and **TD3** (S5b) are also MlpPolicies: their per-step gradient updates are tiny batch-256
 *  forwards that the CPU runs faster than a latency-bound GPU shuttle (measured — same ADR-056 result), so
 *  they train on the CPU and read CPU here. (MuJoCo/BipedalWalker stay GPU-*gated* by step count, but the
 *  device is CPU.) */
export function trainsOnGpu(
  ...
  return false  // SAC / TD3 + every other MlpPolicy trains on CPU (ADR-056)
}
